...
public class PlanController extends FurnitureController implements Controller {
  ...
  private float                           overlapSelectionCycleX = Float.NaN;
  private float                           overlapSelectionCycleY = Float.NaN;
  private int                             overlapSelectionCycleIndex;
  ...
  private float getIndicatorMargin() {
    ...
    if (isTouchLikeInput()) {
      indicatorPixelMagin *= this.preferences.isEnhancedTouchInputEnabled() ? 4 : 3;
    }
   ...
   */
  private boolean isTouchLikeInput() {
    return getPointerTypeLastMousePress() == View.PointerType.TOUCH
        || this.preferences.isEnhancedTouchInputEnabled();
  }

  private Selectable getCycledSelectableItemAt(float x, float y, boolean ignoreGroupsFurniture, boolean advanceCycle) {
    if (!this.preferences.isCycleOverlappingSelectionEnabled()) {
      return getSelectableItemAt(x, y, ignoreGroupsFurniture);
    }
    List<Selectable> items = getSelectableItemsAt(x, y, false, ignoreGroupsFurniture);
    if (items.isEmpty()) {
      this.overlapSelectionCycleX = Float.NaN;
      this.overlapSelectionCycleY = Float.NaN;
      this.overlapSelectionCycleIndex = 0;
      return null;
    }
    if (items.size() == 1) {
      return items.get(0);
    }
    float tolerance = getSelectionMargin() * 2;
    boolean samePosition = !Float.isNaN(this.overlapSelectionCycleX)
        && Math.abs(x - this.overlapSelectionCycleX) <= tolerance
        && Math.abs(y - this.overlapSelectionCycleY) <= tolerance;
    if (!samePosition) {
      this.overlapSelectionCycleX = x;
      this.overlapSelectionCycleY = y;
      this.overlapSelectionCycleIndex = 0;
    } else if (advanceCycle) {
      this.overlapSelectionCycleIndex = (this.overlapSelectionCycleIndex + 1) % items.size();
    }
    return items.get(this.overlapSelectionCycleIndex);
  }
  ...
  private class SelectionMoveState extends ControllerState {
    ...
    public void enter() {
      ...
      if (this.selectionUpdateNeeded
          && !isTouchLikeInput()
          && !preferences.isCycleOverlappingSelectionEnabled()) {
        ...
        // In touch environment or cycle-overlap mode, wait for mouse release to update selection
      }
      List<Selectable> selectedItems = home.getSelectedItems();
    ...
    public void releaseMouse(float x, float y) {
      if (this.mouseMoved) {
        ...
        if (isTouchLikeInput()
          ...
          Selectable itemUnderCursor = getCycledSelectableItemAt(x, y, false, true);
